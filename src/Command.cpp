...
Purpose: Definitions for Command lookup tables.
         Declared extern in Command.h; defined here once so each
         translation unit shares a single construction rather than
...
// STRINGTOSCANCODE is derived at startup by reversing SCANCODETOSTRING.
// This keeps the two maps automatically in sync regardless of which scancode
// constants the current SDL version provides.
static std::unordered_map<std::string, SDL_Scancode> buildStringToScancode() {
    std::unordered_map<std::string, SDL_Scancode> m;
    m.reserve(SCANCODETOSTRING.size());
    for (auto const& [code, name] : SCANCODETOSTRING)
        m.emplace(name, code);
    return m;
}

const std::unordered_map<std::string, SDL_Scancode> STRINGTOSCANCODE =
    buildStringToScancode();
